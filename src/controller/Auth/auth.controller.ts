...
export default class AuthController {
    ...
    login = async (req: Request, res: Response, next: NextFunction) => {
        ...
        if (user.confirmEmail !== true) return next(new ApiError("الرجاء تأكيد الايميل الجامعي", 400));
        if (user.status === "No_Active") return next(new ApiError("لم يتم تفعيل الحساب بعد من قبل المسؤول", 400));
    ...
    //     const user = await this.authService.checkUniversityId(universityId);
    //     if (!user) return next(new ApiError("المستخدم غير موجود", 404));
    //     await sendCode(universityId, res);
